...
if [[ ! -f vars.sh ]]; then
  ...
  # QEMU's CPU hotplug selector is accessed as a 32-bit I/O register by
  # OVMF CpuHotplugSmm. Keep the randomized base 4-byte aligned or edk2's
  # EFI_MM_CPU_IO_PROTOCOL rejects the write with EFI_UNSUPPORTED and the
  # firmware intentionally enters CpuDeadLoop before initializing video.
  cpu=$((     8192          + ((RANDOM%6141)*4) ))
  virtio=$(( 49152 + device ))
...
if (( cpu < 0 || cpu > 65520 || cpu % 4 != 0 )); then
  echo "Invalid CPU hotplug I/O base: $cpu (must be 4-byte aligned and leave room below 0x10000)" >&2
  exit 1
fi
...
file_inteliommu="$(pwd)/qemu/hw/i386/intel_iommu.c"
...
header_q35="$(pwd)/qemu/include/hw/pci-host/q35.h"
header_inteliommu="$(pwd)/qemu/include/hw/i386/intel_iommu.h"
...
if [[ -f "$file_inteliommu" ]]; then rm "$file_inteliommu"; fi
...
if [[ -f "$header_q35" ]]; then rm "$header_q35"; fi
if [[ -f "$header_inteliommu" ]]; then rm "$header_inteliommu"; fi
...
if [[ "${cpu_vendor:1}" == "AuthenticAMD" ]]; then
  ...
  echo "0x293e;                                           -> 0x$hdaudio_8086;  // $hdaname_8086"
...
echo "  $header_q35"
echo "Q35_PSEUDO_BUS_PLATFORM (0xff)                    -> (0x00)"
if ! grep -Fq '#define Q35_PSEUDO_BUS_PLATFORM         (0xff)' "$header_q35"; then
  echo "Unsupported QEMU source: Q35 pseudo IOAPIC bus definition was not found" >&2
  exit 1
fi
# QEMU creates the IOAPIC VT-d address space on the root PCI bus, but stock
# QEMU advertises it in DMAR as bus 0xff.  Advertise the real root-bus
# requester ID instead, keeping interrupt remapping and the DMAR table usable.
sed -i "$header_q35" -Ee 's/#define Q35_PSEUDO_BUS_PLATFORM[[:space:]]+\(0xff\)/#define Q35_PSEUDO_BUS_PLATFORM         (0x00)/'
if grep -Fq '#define Q35_PSEUDO_BUS_PLATFORM         (0xff)' "$header_q35"; then
  echo "Failed to normalize the Q35 IOAPIC DMAR requester bus" >&2
  exit 1
fi

echo "  $file_acpibuild"
echo "DMAR flags                                          -> selectable DMA control platform opt-in"
# Windows Kernel DMA Protection requires firmware to opt in through bit 2 of
# the DMAR table flags. Keep this independent of interrupt remapping and make
# it selectable per VM instead of changing every intel-iommu guest.
if ! grep -Fq 'if (x86_iommu_ir_supported(iommu)) {' "$file_acpibuild"; then
  echo "Unsupported QEMU source: DMAR flags construction was not found" >&2
  exit 1
fi
sed -i "$file_acpibuild" -Ee '/    if \(x86_iommu_ir_supported\(iommu\)\) \{/i\    if (intel_iommu->dma_ctrl_platform_opt_in) {\n        dmar_flags |= 0x4; /* DMA_CTRL_PLATFORM_OPT_IN */\n    }\n'
if ! grep -Fq 'dmar_flags |= 0x4; /* DMA_CTRL_PLATFORM_OPT_IN */' "$file_acpibuild"; then
  echo "Failed to add the DMAR DMA-control platform opt-in flag" >&2
  exit 1
fi

echo "  $header_inteliommu"
echo "IntelIOMMUState                                    -> dma_ctrl_platform_opt_in property state"
if ! grep -Fq 'bool caching_mode;' "$header_inteliommu"; then
  echo "Unsupported QEMU source: IntelIOMMUState caching_mode field was not found" >&2
  exit 1
fi
sed -i "$header_inteliommu" -Ee '/    bool caching_mode;/a\    bool dma_ctrl_platform_opt_in;  /* DMAR DMA control platform opt-in */'

echo "  $file_inteliommu"
echo "intel-iommu properties                             -> dma-control-platform-opt-in"
if ! grep -Fq 'DEFINE_PROP_BOOL("caching-mode", IntelIOMMUState, caching_mode, FALSE),' "$file_inteliommu"; then
  echo "Unsupported QEMU source: intel-iommu property table was not found" >&2
  exit 1
fi
sed -i "$file_inteliommu" -Ee '/    DEFINE_PROP_BOOL\("caching-mode", IntelIOMMUState, caching_mode, FALSE\),/a\    DEFINE_PROP_BOOL("dma-control-platform-opt-in", IntelIOMMUState, dma_ctrl_platform_opt_in, false),'
if ! grep -Fq 'DEFINE_PROP_BOOL("dma-control-platform-opt-in", IntelIOMMUState, dma_ctrl_platform_opt_in, false),' "$file_inteliommu"; then
  echo "Failed to add the intel-iommu DMA-protection property" >&2
  exit 1
fi
...
cd build
KVM_AAVM_BUILD_JOBS="${KVM_AAVM_BUILD_JOBS:-4}"
if ! [[ "$KVM_AAVM_BUILD_JOBS" =~ ^[1-9][0-9]*$ ]]; then
  echo "KVM_AAVM_BUILD_JOBS must be a positive integer." >&2
  exit 2
fi
ninja -j"$KVM_AAVM_BUILD_JOBS" qemu-system-x86_64
iasl "$file_ssdt1"
iasl "$file_ssdt2"
